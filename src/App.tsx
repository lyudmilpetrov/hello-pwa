...
export default function App() {
  ...
  async function importReceipt(getUrl: () => Promise<string>, scanImage: boolean) {
  ...
  async function uploadImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (file) await importReceipt(() => readReceiptUrl(file), true)
  }

  async function submitReceiptUrl(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (receiptUrl.trim()) await importReceipt(async () => receiptUrl, false)
  }
  ...
  return (
    <main className="mx-auto flex min-h-svh w-full max-w-7xl flex-col items-center justify-center gap-6 px-4 py-10 sm:px-6">
      ...
      <div role="group" aria-label="Image actions" className="inline-flex items-center gap-1 rounded-2xl border border-black/10 bg-white p-2 shadow-sm dark:border-white/10 dark:bg-[#1e1e25]">
        <button ref={cameraButton} type="button" onClick={() => { setUploadError(null); setCameraOpen(true) }} disabled={isBusy || cameraOpen} aria-haspopup="dialog" className="inline-flex min-h-12 items-center justify-center gap-2 whitespace-nowrap rounded-xl px-3 text-sm font-medium transition-colors hover:bg-violet-50 hover:text-violet-700 disabled:cursor-wait disabled:opacity-50 sm:px-5 dark:hover:bg-violet-400/10 dark:hover:text-violet-300">
      ...
      <form onSubmit={submitReceiptUrl} aria-label="Import receipt link" className="flex w-full max-w-xl flex-col gap-2 sm:flex-row sm:items-end">
        <div className="min-w-0 flex-1">
          ...
          <input id="receipt-url" type="url" value={receiptUrl} onChange={(event) => setReceiptUrl(event.currentTarget.value)} placeholder="https://tax.salyk.kg/…" disabled={isBusy} required className="min-h-11 w-full rounded-xl border border-black/15 bg-white px-3 text-sm disabled:opacity-50 dark:border-white/15 dark:bg-[#1e1e25]" />
        </div>
        <button type="submit" disabled={isBusy || !receiptUrl.trim()} className="min-h-11 rounded-xl bg-violet-700 px-5 text-sm font-medium text-white transition-colors hover:bg-violet-800 disabled:cursor-default disabled:opacity-50 dark:bg-violet-500 dark:hover:bg-violet-600">Import receipt</button>
      </form>
      ...
      {cameraOpen && <CameraScanner onClose={() => {
        setCameraOpen(false)
